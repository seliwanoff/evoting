...
<section class="banner major banner-dark">
	<div class="banner-wrapper">
		...
		?>
			<h2>Cast your vote for your favourite party</h2>
			<p>Kindly, select your favorite for <?php echo $fetch['post'] ; ?> </p>
			<p>Note: You can only vote for each post</p>
					</div>
...
<section style="padding: 10px; margin:20px; width:100%;max-width:500px;margin:0px auto">

	<div style="width:100%; margin-top: 60px;">
    
		<label for="post">Vote your Party:</label>
		<form method="post">
		...
		if(isset($_POST['submitvote'])){
			include('../asset/connect.php');
			$getme = $_POST['vote'];
			$selectuser = "SELECT * FROM `newregister` WHERE (`phone`='".$_SESSION['id']."' OR `email`='".$_SESSION['id']."')";
			$connect = mysqli_query($link,$selectuser);
			$fetchs = mysqli_fetch_array($connect);
			$voterid = $fetchs['voterid'];
			$selectpost = "SELECT * FROM `parties` WHERE `id`='".$getme."'";
			$cons = mysqli_query($link,$selectpost);
			$fe = mysqli_fetch_array($cons);
			$pname = $fe['pparties'];
			$status='true';
			$select = "SELECT * FROM `polling` WHERE  `user`= '".$voterid."'";
			$links = mysqli_query($link,$select);

			$nums = mysqli_num_rows($links);
			if($nums>0){
				?>
				<div class="alert alert-danger" role="alert">
                 You have cast your vote. Kindly wait for the result as you cannot cast another vote.
                </div>
				<?php
			}else{
				$insert = "INSERT INTO `polling`(`pd`, `pname`, `user`, `status`)
				 VALUES ('".$getme."','".$pname."','".$voterid."','".$status."')";
				
				if(mysqli_query($link,$insert)){
					?>
						 <div class="alert alert-success" role="alert">
							Vote casted sucessfully. Kindly check the result of the vote. Thanks
						</div>
					<?php
				}else{
					?>
						<div class="alert alert-danger" role="alert">
						Error Casting Vote. Try again.
					</div>
					<?php
				}
			}
		...
		?>
			...
			include('../asset/connect.php');
			$type = $_GET['type'];
			$select  ="SELECT * FROM `parties` WHERE `type`='".$type."'";
			$res = mysqli_query($link,$select);
			$count = mysqli_num_rows($res);
			...
			if($count){
				while($fetch= mysqli_fetch_array($res)){
					?>
						<div class="card-barner" partyid="<?php echo $fetch['id'];  ?>" pid="party<?php echo $fetch['id'] ?>">
						<input class="party<?php echo $fetch['id']?>" type="radio" name="vote" value="<?php echo $fetch['id']?>" pid="party<?php echo $fetch['id'] ?>"/> <?php echo $fetch['pparties'];?>
				...
				?>
				<div class="alert alert-danger" role="alert">
                No Party or No election has been scheduled for this post.Check back later.
                </div>
				<?php
			}
			?>
		...
		if($count){
			?>
					<input class="vote-submit" type="submit" style="margin-top:50px;background-color: #0693e3;width:200px;padding: 10px;color:#fff;border:1px solid #0693e3" class="getaspirant" value="Vote" name="submitvote">
	...
	$(document).ready(function(){
		...
		if(datas==null){
          ...
          let type = data;
            ...
            let  url = `http://localhost/apis/getparties.php?type=${type}`;
       let h = new Headers();
       var html ="";
	 ...
	 html+=`<div class="card-barner" partyid="${data[a].id }" pid="party${data[a].id }" > 
	 <input class="party${data[a].id }" type="radio" name="vote" value="${data[a].id }" pid="party${data[a].id }"/>`+data[a].pparties +"</div>";
	$('.form-select').html(html).val(data[a].type)

	$('.card-barner').click(function(){
		var id = $(this).attr('pid')
		var check =	$('.'+id).prop('checked',true).val()
